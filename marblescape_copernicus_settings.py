...
class CopernicusSettings:
    ...
    def _set_date_choices(self, selected):
        ...
        self._date_by_label = {LATEST_LABEL: "latest", **{value: value for value in values[1:]}}
        self._date_combo.configure(values=values, state="readonly")
        self._date_var.set(LATEST_LABEL if selected == "latest" else selected)
    ...
    def _set_coverage_controls(self, mode, lookback_days):
        ...
        self._lookback_combo.configure(
            state="readonly" if mode == "fill_gaps" else "disabled"
        )
    ...
    def _select_coverage(self, _event=None):
        if self._updating:
            return
        mode = COVERAGE_LABELS.get(self._coverage_var.get(), "")
        self._lookback_combo.configure(
            state="readonly" if mode == "fill_gaps" else "disabled"
        )
        self._changed()
    ...
    def _poll(self):
        ...
        try:
            while True:
                ...
                values = [LATEST_LABEL, *dates]
                if selected != "latest" and selected not in dates:
